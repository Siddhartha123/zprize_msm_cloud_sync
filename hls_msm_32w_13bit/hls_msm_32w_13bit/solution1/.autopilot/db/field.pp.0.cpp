...
void delay(int x) {
    ...
    { VITIS_LOOP_15_1: for(unsigned __i__=0; __i__<x; ++__i__) { _ssdm_Unroll(0,0,0, ""); _ssdm_op_Wait(1); } };
...
#pragma HLS inline
 ap_int<256> a_prim = m_transform(a);
    ap_int<256> b_prim = m_transform(b);
    ap_int<256> sum_prim = a_prim + b_prim;
    coeff_t sum = m_reduce(sum_prim);
    return sum;
}
