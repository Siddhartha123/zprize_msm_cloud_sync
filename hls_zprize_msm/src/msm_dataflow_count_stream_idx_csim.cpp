...
#pragma HLS array_partition variable = GBUFF_P2D type = cyclic factor = 43 dim = 1
    chunk_t nibble_K_input_process;
    chunk_t rem_nibble_K;
...
#pragma HLS array_partition variable = num_padd_ops_local type = cyclic factor = 43 dim = 1
    ...
    for (int i = 0; i < NUM_CHUNKS; i++) std::cout << num_padd_ops_local[i] << ", ";
...
#pragma HLS unroll
            nibble_K_input_process = K_arr[i](((ni + 1) * 6) - 1, ni * 6);
            // csim print
            // std::cout << "[nibble] " << nibble_K_input_process << "\n";
            BFIFO[ni].write((nibble_K_input_process, i));
            // std::cout << "[chunk " << ni << "][BFIFO] write " << (nibble_K_input_process, i)
            //           << "\n";
        }
        rem_nibble_K = K_arr[i](fr_t::width - 1, (NUM_CHUNKS - 1) * 6);
        BFIFO[NUM_CHUNKS - 1].write((rem_nibble_K, i));
        // std::cout << "[chunk " << NUM_CHUNKS - 1 << "][BFIFO] write " << (rem_nibble_K, i) <<
        // "\n";
    }

    // Bucket process variables declaration
    bls12_377_idx_k_t data[NUM_CHUNKS];
    bool valid_data[NUM_CHUNKS];
    memset(valid_data, 0, sizeof(valid_data));
    N_t count_if[NUM_CHUNKS] = 0, count_padd_of[NUM_CHUNKS] = 0;
    chunk_t nibble_K[NUM_CHUNKS];
    // ---- Buckets ----
    bool fill_B[NUM_CHUNKS][TWO_RAISED_CHUNK_SIZE];
    memset(fill_B, true, sizeof(fill_B));
    hls::stream<bls12_377_idx_t> B[NUM_CHUNKS][TWO_RAISED_CHUNK_SIZE];
    hls::stream<bls12_377_idx_k_t> OVFIFO[NUM_CHUNKS];
    bls12_377_idx_t result[NUM_CHUNKS];
    // bucket-wise padd count
    N_t padd_count[NUM_CHUNKS][TWO_RAISED_CHUNK_SIZE];
    memset(padd_count, 0, sizeof(padd_count));

    // ---- Padd operations ----
    fp_t iter_padd_ops = 0;
    bool padd_done = false;
    ...
    std::cout << "\n pippenger algorithm starts\n";
    int tick = 0;
    int flag = 1;
    while (++tick < 1500) {
        for (int ni = NUM_CHUNKS - 1; ni < NUM_CHUNKS; ni = ni + 1) {
            std::cout << "[debug]chunk position " << ni << std::endl;
#pragma HLS unroll
        bucket_process : {
            // while ((count_if[ni] < NUM_POINTS) || (count_padd_of[ni] < num_padd_ops_local[ni]))
            {  // global_loop
                if (count_padd_of[ni] < num_padd_ops_local[ni] &&
                    !padd_output_bucket_fifo[ni].empty()) {
                    data[ni] = padd_output_bucket_fifo[ni].read();
                    count_padd_of[ni] += 1;
                    valid_data[ni] = true;
                    // Check for last bucket element here.
                    // update padd count here
                    // if last element, put in B_i and change valid to false
                    padd_count[ni][data[ni](NIBBLE_RANGE_IDX)] += 1;
                    if (padd_count[ni][data[ni](NIBBLE_RANGE_IDX)] ==
                        cnt_bucket_chunks[ni][data[ni](NIBBLE_RANGE_IDX)] - 1) {
                        GBUFF_P2D[ni][data[ni](NIBBLE_RANGE_IDX)] =
                            data[ni](bls12_377_idx_t::width - 1, 0);
                        valid_data[ni] = false;
                    }
                } else if (count_if[ni] < NUM_POINTS && !BFIFO[ni].empty()) {
                    data[ni] = BFIFO[ni].read();
                    std::cout << "[tick " << tick << "][chunk " << ni << "][BFIFO] read "
                              << data[ni](NIBBLE_RANGE_IDX) << " "
                              << bls12_377_p(data[ni](bls12_377_idx_t::width - 1, 0)).x << "\n";
                    count_if[ni] += 1;
                    valid_data[ni] = true;
                    // take care of single entry buckets i.e. cnt_bucket_chunks[ni]==1
                    if (cnt_bucket_chunks[ni][data[ni](NIBBLE_RANGE_IDX)] == 1) {
                        GBUFF_P2D[ni][data[ni](NIBBLE_RANGE_IDX)] =
                            data[ni](bls12_377_idx_t::width - 1, 0);
                        valid_data[ni] = false;
                    }
                ...
                if (valid_data[ni]) {
                    nibble_K[ni] = data[ni](NIBBLE_RANGE_IDX);
                    if (nibble_K[ni] != 0) {
                        if (fill_B[ni][nibble_K[ni]]) {
                            B[ni][nibble_K[ni]].write(data[ni](bls12_377_idx_t::width - 1, 0));
                            std::cout << "[tick " << tick << "][chunk " << ni << "][Bucket "
                                      << nibble_K[ni] << "] write "
                                      << bls12_377_p(data[ni](bls12_377_idx_t::width - 1, 0)).x
                                      << "\n";
                        } else {
                            OVFIFO[ni].write(data[ni]);
                            std::cout << "[tick " << tick << "][chunk " << ni << "][OVFIFO] write "
                                      << nibble_K[ni] << " "
                                      << bls12_377_p(data[ni](bls12_377_idx_t::width - 1, 0)).x
                                      << "\n";
                        }
                        fill_B[ni][nibble_K[ni]] = !fill_B[ni][nibble_K[ni]];
                    }
                }
            }

        msm_arr_dataflow_3:
            // for (int i = 0; i < num_padd_ops_local[ni]; i++)
            if (!OVFIFO[ni].empty()) {  // global_loop
                data[ni] = OVFIFO[ni].read();
                nibble_K[ni] = data[ni](NIBBLE_RANGE_IDX);
                result[ni] = B[ni][nibble_K[ni]].read();
                CFIFO[ni].write(
                    (nibble_K[ni], result[ni], data[ni](bls12_377_idx_t::width - 1, 0)));
                std::cout << "[tick " << tick << "][chunk " << ni << "][CFIFO] write "
                          << nibble_K[ni] << " " << result[ni] << " "
                          << data[ni](bls12_377_idx_t::width - 1, 0) << "\n";
            }
        }
        if(flag==1)
...
#pragma HLS bind_storage variable = scratch_pad type = RAM_2P
    // TODO: separate bucket and loop2and3 processes and write to two queues
    padd_input_load_unit : {
#pragma HLS inline
        ...
        // while (itr_bucket < total_num_padd_ops || itr_loop2_3 < (TWO_RAISED_CHUNK_SIZE - 1) *
        // NUM_CHUNKS * 2 + NUM_CHUNKS - 2)
        {  // global_loop
#pragma HLS dependence variable = scratch_pad RAW false
            if (itr_bucket < total_num_padd_ops && (!padd_input_bucket_fifo.empty())) {
#pragma HLS dependence variable = scratch_pad RAW false
                ...
                // NOTE: p1_addr, p2_addr can be locations from GBUFF_P as well as scratch pad, use
                // MSB of address to distinguish between both

                padd_input_fifo.write((chunk_num, nibble_K, p1_x, p1_y, p1_z, p2_x, p2_y, p2_z));
                std::cout << "[tick " << tick << "][chunk " << chunk_num
                          << "][padd_input_fifo] write " << nibble_K << std::endl;
                itr_bucket++;
            } else if ((itr_loop2_3 <
                        (TWO_RAISED_CHUNK_SIZE - 1) * NUM_CHUNKS * 2 + NUM_CHUNKS - 2) &&
                       (!padd_input_loop_2and3_fifo.empty())) {
#pragma HLS dependence variable = scratch_pad RAW false
                ...
                itr_loop2_3++;
                std::cout << "[tick " << tick << "][chunk " << 0x3f << "][padd_input_fifo] write "
                          << 0 << std::endl;
            }
        }
    }
    ...
    msm_arr_dataflow_padd:
        // for (int i = 0;i < total_num_padd_ops + (TWO_RAISED_CHUNK_SIZE - 1) * NUM_CHUNKS * 2 +
        // NUM_CHUNKS - 2;i++)
        if (!padd_input_fifo.empty()) {  // global_loop
...
#pragma HLS inline
        ...
        // for (int i = 0;i < total_num_padd_ops + (TWO_RAISED_CHUNK_SIZE - 1) * NUM_CHUNKS * 2 +
        // NUM_CHUNKS - 2;i++)
        if (!padd_output_fifo.empty()) {  // global_loop
            ...
            std::cout << "[tick " << tick << "][chunk " << chunk_num << "][scratch_pad] write "
                      << nibble_K << " addr " << store_count << std::endl;
